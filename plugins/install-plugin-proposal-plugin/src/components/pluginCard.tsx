...
import {Flex, Button, HStack, Stack, Tag, Text, Link, Heading, Spacer, useToast, AlertStatus} from '@chakra-ui/react';
...
const PluginCard = ({daoId, plugin_info, installed, showDetail = true}: PluginCardProps) => {
    ...
    const onUninstallPlugin = async () => {
        showToast('Under development, stay tuned', 'error')
    }
    ...
    const onInstallPlugin = async () => {
        try {
            ...
            showToast(`Create install plugin proposal success, transactionHash: ${transactionHash}`, 'success')
        } catch (err: any) {
            console.log(err);
            showToast(`Create install plugin proposal failed, error: ${err.message}`, 'success')
        }
    }
        ...
        <ContentBox>
            <Stack spacing={4}>
                <Flex as={HStack} spacing={2} align='center'>
                    ...
                    <Spacer/>
                    {stared ? (
                        <Button leftIcon={<AiFillStar/>} size='xs' colorScheme='orange' variant='solid'
                                onClick={onSwitchStar}>Stared {starCount}</Button>
                    ) : (
                        <Button leftIcon={<AiOutlineStar/>} size='xs' colorScheme='orange' variant='solid'
                                onClick={onSwitchStar}>Star {starCount}</Button>
                    )
                    }
                </Flex>
                <Flex as={HStack} spacing={2} align='center'>
